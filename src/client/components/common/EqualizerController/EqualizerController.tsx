...
const EqualizerInput = ({ name, value, index, onChange, onInput }: EqualizerInputProps) => {
    return (
        <>
            ...
            <div
                className={styled.equalizerHorizontalLabel}
                style={{ gridArea: `5 / ${index + 3} / 6 / ${index + 4}` }}
            >
                <span>{name}</span>
            </div>
        </>
...
export const EqualizerController = ({ defaultEqualizerValue, onValueChange }: EqualizerControllerProps) => {
    ...
    useEffect(() => {
        if (pathRef.current && pathDrawRef.current && svgWidth !== 0 && svgHeight !== 0) {
            ...
            const points = Object.values(equalizerValue).map((value, index) => {
    ...
    return (
        <div className={styled.equalizerContainer}>
            <span className={styled.equalizerVerticalLabel} style={{ gridArea: "1 / 1 / 2 / 2" }}>
                +12dB
            </span>
            <span className={styled.equalizerVerticalLabel} style={{ gridArea: "3 / 1 / 4 / 2" }}>
                -12dB
            </span>
            ...
            <svg className={styled.equalizerSvg} ref={svgRef}>
                ...
                <rect
                    ...
                    width="100%"
                    height="100%"
                    fill="url(#equalizerGradient)"
                    clipPath="url(#equalizerClipPath)"
                />
            </svg>
        </div>
    );
