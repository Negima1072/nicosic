...
export const SearchPage = () => {
    ...
    const [searchQuery, searchType, queryType, sortKey, sortOrder, listType] = useMemo(() => {
        ...
        if (_searchType === "video") {
            ...
            setSelectedSortSettingIndex(
                videoSortSettings.findIndex((item) => item.key === _sortKey && item.order === _sortOrder),
            );
            ...
            setSelectedSortSettingIndex(
                listSortSettings.findIndex((item) => item.key === _sortKey && item.order === _sortOrder),
            );
            if (_queryType === "tag") _queryType = "keyword";
        ...
        return [
            _searchQuery,
            _searchType as SearchType,
            _queryType as SearchQueryType,
            _sortKey,
            _sortOrder,
            _listType as SearchListType,
        ];
    }, [searchParams]);
    ...
    const setSearchSetting = (index: number) => {
        if (searchType === "video") {
            setSearchParams({
                ...
                type: searchType,
                queryType,
                sortKey: videoSortSettings[index].key,
                sortOrder: videoSortSettings[index].order,
            });
            ...
            setSearchParams({
                ...
                type: searchType,
                queryType,
                sortKey: userSortSetting[index].key,
                sortOrder,
            });
            ...
            setSearchParams({
                ...
                type: searchType,
                queryType,
                sortKey: listSortSettings[index].key,
                sortOrder: listSortSettings[index].order,
            });
        }
                    ...
                    <div className={styled.searchSettingSelector}>
                        <div className={styled.searchTypeSelector}>
                            <div
                                ...
                                className={searchType === "video" && queryType === "keyword" ? styled.selected : ""}
                            >
                                キーワード
                            </div>
                            <div
                                onClick={() => setSearchType("tag")}
                                className={searchType === "video" && queryType === "tag" ? styled.selected : ""}
                            >
                                タグ
                            </div>
                            <div
                                onClick={() => setSearchType("user")}
                                className={searchType === "user" ? styled.selected : ""}
                            >
                                ユーザー
                            </div>
                            <div
                                onClick={() => setSearchType("mylist")}
                                className={searchType === "list" && listType === "mylist" ? styled.selected : ""}
                            >
                                マイリスト
                            </div>
                            <div
                                onClick={() => setSearchType("series")}
                                className={searchType === "list" && listType === "series" ? styled.selected : ""}
                            >
                                シリーズ
                            </div>
                        </div>
                        <select
                            className={styled.searchSortSelector}
                            value={selectedSortSettingIndex}
                            onChange={(e) => setSearchSetting(parseInt(e.target.value))}
                        >
                            {searchType === "video" &&
                                videoSortSettings.map((item, index) => (
                                    <option key={index} value={index}>
                                        {item.label}
                                    </option>
                                ))}
                            {searchType === "user" &&
                                userSortSetting.map((item, index) => (
                                    <option key={index} value={index}>
                                        {item.label}
                                    </option>
                                ))}
                            {searchType === "list" &&
                                listSortSettings.map((item, index) => (
                                    <option key={index} value={index}>
                                        {item.label}
                                    </option>
                                ))}
                ...
                <>
                    {searchType === "video" && (
                        <VideoSearchViewer
                            ...
                            queryType={queryType}
                            sortKey={sortKey as SearchVideoSortKey}
                    ...
                    {searchType === "list" && (
                        <ListSearchViewer
                            searchQuery={searchQuery}
                            listType={listType}
                            sortKey={sortKey as SearchListSortKey}
                            sortOrder={sortOrder as SearchSortOrder}
                        />
                    )}
                </>
